...
- **Testing stats**: the disclaimer screen states that continuing shares name, score, approximate region (city/country from IP), and connection type/speed to a Google Sheet you control. Continuing past the disclaimer counts as agreeing to this -- there's no separate opt-in checkbox. See `google-apps-script/README.md` to wire this up (optional — the game works fully without it, the disclaimer just becomes moot if `COLLECT_ENDPOINT` is never set).
